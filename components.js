...
// Function to create an anime card element (Moved here for reusability across all pages for search results etc.)
function createAnimeCard(anime) {
    ...
    // Using a class from the page-specific CSS that defines card dimensions/appearance for main grids
    // This allows page-specific CSS to override default component styles if needed for specific layouts
    ...
    // Only add action buttons for main display, not for search results which use a different card layout
    const actions = document.createElement('div');
    actions.className = 'card-actions';

    const episodeBtn = document.createElement('button');
    episodeBtn.className = 'episode-btn';
    episodeBtn.textContent = anime.episode;
...
function getAnimeStatus(animeTitle) { // Input is anime title string
    ...
    // If anime object has a status property, use it.
    if (anime && anime.status) {
        return anime.status;
    }

    // Backup: If anime object doesn't have status or not found, check a predefined list
    const completeList = ["Naruto", "Dragon Ball Z", "Fullmetal Alchemist", "Death Note", "Code Geass", "Attack on Titan", "Demon Slayer", "Hunter X Hunter", "Mob Psycho 100", "Vinland Saga", "Fire Force", "Black Clover", "Dr. Stone", "Steins;Gate", "Chainsaw Man", "Solo Leveling", "Frieren: Beyond Journey's End", "The Apothecary Diaries", "To Be Hero X", "Perfect World Season 4", "Tales of Herding Gods", "Renegade Immortal"];
    return completeList.includes(animeTitle) ? "Complete" : "Ongoing";
}
...
function toggleSearchBox() {
    ...
    if (box) { // Check if searchBox exists
        if (box.style.display === 'block') {
            ...
            // Specific logic for all-anime.html: reset pagination if present
            // Check if displayCurrentPage and relevant global variables exist on the current page
...
function displayCurrentPage(animeListToDisplay) {
    ...
    currentItems.forEach(anime => {
        const animeCardElement = createAnimeCard(anime); // Use common createAnimeCard
...
// components.js (relevant part for displayAnimeDetails)
    ...
    container.innerHTML = `
        ...
        <section class="watch-anime-section">
            ...
            <div class="video-player-placeholder" id="videoPlayerPlaceholder">
                Click on an episode below to watch.
            </div>
        </section>
